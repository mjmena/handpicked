...
fn Touch(touch_point: ReadSignal<TouchPoint>, radius: ReadSignal<f64>) -> impl IntoView {
    ...
    view! {
        <svg x={move || position().0-radius()} y={move || position().1-radius()} height={move || size()} width={move || size()}>
        <defs>
            <radialGradient id=move || touch_point().id>
            <stop offset=move || format!("{}%", radius() / 10.0) stop-color="gold" />
            <stop offset="95%" stop-color="red" />
            </radialGradient>
        </defs>
          <circle r=move || radius() cx=move || radius() cy=move || radius() fill=format!("url('#{}')", touch_point().id)
          />
        </svg>
    }
}
